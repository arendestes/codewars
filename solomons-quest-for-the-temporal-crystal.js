// Background
// In another dimension, there exist two immortal brothers: Solomon 
// and Goromon. As sworn loyal subjects to the time elemental, Chronixus, 
// both Solomon and Goromon were granted the power to create temporal 
// folds. By sliding through these temporal folds, one can gain entry 
// to parallel dimensions where time moves relatively faster or slower.

// Goromon grew dissatisfied and one day betrayed Chronixus by stealing 
// the Temporal Crystal, an artifact used to maintain the time continuum. 
// Chronixus summoned Solomon and gave him the task of tracking down 
// Goromon and retrieving the Temporal Crystal.

// Using a map given to Solomon by Chronixus, you must find Goromon's 
// precise location.
...
// Time Dilation: With each additional layer of time dilation entered, 
// time slows by a factor of 2. At layer 0, time passes normally. At layer 1, 
// time passes at half the rate of layer 0. At layer 2, time passes at half 
// the rate of layer 1, and therefore one quarter the rate of layer 0.

// Directions are as follow: 0 = North, 1 = East, 2 = South, 3 = West

// Distance Traveled: This represents the distance traveled relative to the 
// current time dilation layer. 
...
function solomonsQuest(ar) {
    ...
    for ( let i = 0; i < ar.length; i ++){
        dilationLevel += ar[i][0];
        const movementFactor = Math.pow(2, dilationLevel);
        const movement = ar[i][2] * movementFactor;
        const direction = ar[i][1];
        if ( direction === 0 ) {
            coordinates = [coordinates[0], coordinates[1] + movement];
        } else if ( direction === 1 ) {
            coordinates = [coordinates[0] + movement, coordinates[1]];
        } else if ( direction === 2 ) {
            coordinates = [coordinates[0], coordinates[1] - movement];
        } else {
            coordinates = [coordinates[0] - movement, coordinates[1]];
...
const result = solomonsQuest([[1,3,5],[2,0,10],[-3,1,4],[4,2,4],[1,1,5],[-3,0,12],[2,1,12],[-2,2,6]]);
